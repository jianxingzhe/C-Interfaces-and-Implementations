...
char *Str_reserve(const char *s,int i,int j){
	char *str,*p;
	convert(s,i,j);
	p=str=ALLOC(j-i+1);
	while(j>i)
...
int Str_char  (const char *s,int i,int j,int c){
	convert(s,i,j);
	for(;i<j;i++)
		if(s[i]==c)
			return i+1;
...
int Str_any(const char *s,int i,int j,const char *set){
	int len;
