...
static void cvt_f(int code,va_list *app,int put(int c,void *cl),void *cl,
				  unsigned char flags[],int width,int precision){
	/*******************************************************************************
	 * DBL_MAX_10_EXP表示double表示的最大的十进制指数值，如果用%.99f转换DBL_MAX，结果
	 * 的位数情况：小数点之前可能有DBL_MAX_10_EXP+1个小数、小数点、小数点之后99个数位
	 * 结果的0字符
	 ******************************************************************************/
...
void Fmt_fprintf(FILE *stream,const char *fmt,...){
	va_list ap;
	va_start(ap,fmt);
...
T Fmt_register(int code, T newcvt){
	T old;
	assert(0<old && code <(int)(sizeof (cvt)/sizeof (cvt[0])));
	old = cvt[code];
	cvt[code] = newcvt;
	return old;
}
